...
function ComboSelect({ value, onChange, options, onAddNew, emptyLabel, showAddNew = true }) {
  ...
  return (
    ...
    >
      ...
      {showAddNew && <option value="__add_new__">+ Add new…</option>}
    </Select>
  );
}
  ...
  return (
    <div style={{ display: "grid", gap: 6 }}>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 6, flexWrap: "wrap" }}>
        {adding ? (
          <>
            <Input autoFocus value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addNow(); } if (e.key === "Escape") { setAdding(false); setDraft(""); } }} placeholder={`New ${String(label || "material").toLowerCase()}…`} style={{ flex: 1, minWidth: 140, padding: "6px 8px", fontSize: 11.5 }} />
            <Button small type="button" onClick={addNow}>Add</Button>
            <Button small type="button" variant="ghost" onClick={() => { setAdding(false); setDraft(""); }}><X size={12} /></Button>
          </>
        ) : (
          <>
    ...
    <div className="erp-shell" style={{ fontFamily: FONT_BODY, background: COLORS.bg, color: COLORS.ink }}>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        @media (max-width: 900px) { .erp-sidebar { width: 72px !important; flex-basis: 72px !important; padding-left: 8px !important; padding-right: 8px !important; } .erp-sidebar > div:first-child > div:not(:first-child), .erp-sidebar button { font-size: 0 !important; justify-content: center !important; } .erp-sidebar button svg { margin: 0 !important; } }
        .sample-kanban-board {
        ...
        @media (max-width: 600px) {
          ...
          .ai-assistant-overlay { padding:10px; }
          .ai-assistant-panel { width:100%; height:calc(100vh - 20px); border-radius:16px; }
        }
...
function FloatingAIAssistant({ samples, tasks, customers, materialPreps, orders, customerName, productTypeName }) {
  ...
  const sampleStage = (s) => s.stage || "Request Received";
  const compactDate = (v) => v || "—";
  const today = new Date().toISOString().slice(0, 10);

  const buildContext = () => {
    const customerById = new Map(customers.map(c => [c.id, c.name || c.id]));
    ...
    return {
      ...
      summary: {
        ...
        samplesByStage: SAMPLE_STAGES.reduce((acc, stage) => { acc[stage] = samples.filter(s => sampleStage(s) === stage).length; return acc; }, {}),
        openTasks: tasks.filter(t => t.status !== "Done").length,
        overdueTasks: tasks.filter(t => t.status !== "Done" && t.deadline && t.deadline < today).length,
        materialRows: materialPreps.length,
      },
      samples: samples.map(s => ({
        ...
        targetDate: compactDate(s.targetDate),
        priority: s.priority || "",
        overallStatus: s.overallStatus || "",
        orderId: s.orderId || "",
        materialReadiness: (() => {
          const rows = materialPreps.filter(p => p.sampleId === s.id);
          const done = rows.filter(p => String(p.status || "").toLowerCase() === "done").length;
          return rows.length ? `${done}/${rows.length}` : "No plan";
        })(),
      })),
      tasks: tasks.map(t => ({
        id: t.id,
        ...
        status: t.status || "",
        priority: t.priority || "",
        deadline: compactDate(t.deadline),
        referencePerson: t.referencePerson || "",
        sampleId: t.sampleId || "",
        sampleName: sampleById.get(t.sampleId)?.name || "",
        description: t.description || "",
        note: t.note || "",
      })),
      orders: orders.map(o => ({ id: o.id, customer: customerById.get(o.customerId) || "—", stage: o.stage || "", status: o.status || "" })),
    };
  };
  ...
  const renderInlineMarkdown = (value, keyPrefix = "md") => {
  ...
  const MarkdownMessage = ({ text }) => {
    ...
    while (i < lines.length) {
      ...
      if (heading) {
        ...
        blocks.push(<Tag key={`h-${i}`}>{renderInlineMarkdown(heading[1], `h-${i}`)}</Tag>);
        i += 1;
        continue;
      }

      const isTable = i + 1 < lines.length && /^\s*\|?.+\|.+\|?\s*$/.test(line) && /^\s*\|?\s*:?-{3,}:?\s*(\|\s*:?-{3,}:?\s*)+\|?\s*$/.test(lines[i + 1]);
      if (isTable) {
        ...
        blocks.push(
          <div className="ai-md-table-wrap" key={`table-${i}`}>
            <table className="ai-md-table">
              <thead><tr>{headers.map((h, idx) => <th key={idx}>{renderInlineMarkdown(h, `th-${i}-${idx}`)}</th>)}</tr></thead>
              <tbody>{rows.map((row, r) => <tr key={r}>{headers.map((_, c) => <td key={c}>{renderInlineMarkdown(row[c] || "", `td-${i}-${r}-${c}`)}</td>)}</tr>)}</tbody>
            </table>
          </div>
        );
        continue;
      ...
      if (/^\s*(?:[-*•])\s+/.test(line)) {
        ...
        blocks.push(<ul key={`ul-${start}`}>{items.map((item, idx) => <li key={idx}>{renderInlineMarkdown(item, `ul-${start}-${idx}`)}</li>)}</ul>);
        continue;
      }
      ...
      if (/^\s*\d+[.)]\s+/.test(line)) {
        ...
        blocks.push(<ol key={`ol-${start}`}>{items.map((item, idx) => <li key={idx}>{renderInlineMarkdown(item, `ol-${start}-${idx}`)}</li>)}</ol>);
        continue;
      }
      ...
      while (i < lines.length) {
        const next = lines[i];
        const t = next.trim();
        if (!t || /^#{1,3}\s+/.test(t) || /^\s*(?:[-*•])\s+/.test(next) || /^\s*\d+[.)]\s+/.test(next) || /^\s*([-*_])(?:\s*\1){2,}\s*$/.test(t)) break;
        paragraph.push(next);
        i += 1;
      }
      blocks.push(<p key={`p-${start}`}>{paragraph.map((part, idx) => <React.Fragment key={idx}>{idx ? <br /> : null}{renderInlineMarkdown(part, `p-${start}-${idx}`)}</React.Fragment>)}</p>);
    }
  ...
  const ask = async (text = question) => {
    ...
    try {
      ...
      let data;
      try { data = JSON.parse(raw); } catch { throw new Error(raw.slice(0, 500) || `HTTP ${res.status}`); }
      if (!res.ok) throw new Error(data.error || `AI request failed (${res.status})`);
      setMessages(prev => [...prev, { role: "assistant", text: data.answer || "I couldn't find an answer in the current ERP data." }]);
    } catch (err) {
      setMessages(prev => [...prev, { role: "assistant", error: true, text: `AI Assistant error: ${err.message}` }]);
  ...
  return (
    <>
      <button className="floating-ai-button" onClick={() => setOpen(true)} aria-label="Open ERP AI Assistant">
        <Sparkles size={19} strokeWidth={2.2} />
        <span>AI Assistant</span>
      </button>

      {open && (
        <div className="ai-assistant-overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) setOpen(false); }}>
          <div className="ai-assistant-panel" role="dialog" aria-modal="true" aria-label="ERP AI Assistant">
            ...
            <div className="ai-assistant-body">
              {!messages.length && (
                <div className="ai-welcome">
                  <div className="ai-welcome-icon"><Sparkles size={24} /></div>
                  <div className="ai-welcome-title">What do you want to know?</div>
                  <div className="ai-welcome-copy">I can answer from the ERP data currently loaded on this screen. I won't change any data.</div>
              ...
              {messages.map((m, i) => (
                <div key={i} className={`ai-message-row ${m.role}`}>
                  <div className={`ai-message ${m.error ? "error" : ""}`}>{m.error || m.role === "user" ? m.text : <MarkdownMessage text={m.text} />}</div>
                </div>
              ))}
              {loading && <div className="ai-message-row assistant"><div className="ai-message ai-thinking">Thinking…</div></div>}
            </div>
            ...
            <div className="ai-assistant-footer">
              <div className="ai-input-wrap">
                <textarea
                  ...
                  disabled={loading}
                />
                <button onClick={() => ask()} disabled={!question.trim() || loading} aria-label="Send"><span>↑</span></button>
              </div>
              <div className="ai-disclaimer">Read-only assistant · Answers are based on the ERP data currently loaded.</div>
            </div>
          </div>
        </div>
      )}
...
function SamplesView({ samples, saveSamples, customers, customerName, productTypes, productTypeName, orders, materialLists, saveMaterialList, materialPreps, saveMaterialPreps, tasks, saveTasks }) {
  ...
  const applyAIDescription = async (ai, createMissing) => {
    ...
    if (ai.__imagePreview) {
      try {
        const blobResponse = await fetch(ai.__imagePreview);
  ...
  const importMaterialListRows = (listKey, rows, options = {}) => {
    const tab = MATERIAL_LIST_TABS.find((t) => t.key === listKey);
    if (!tab) return;
    const current = materialLists[listKey] || [];
    const base = options.replaceExisting ? [] : [...current];
    const byCode = new Map(base.filter((m) => m.code).map((m) => [String(m.code).trim().toLowerCase(), String(m.id || "").trim()]));
    ...
    for (const raw of rows || []) {
      ...
      if (code) byCode.set(code.toLowerCase(), id);
    }
    saveMaterialList(listKey, generated);
    setMaterialImportKey(null);
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      {(() => {
        ...
        const completed = samples.filter((s) => String(s.overallStatus || "").toLowerCase() === "completed" || normalizeSampleWorkflowStage(s.stage) === "Shipping" && s.completedDate).length;
        const inProgress = samples.filter((s) => !completed && normalizeSampleWorkflowStage(s.stage) !== "Request Received").length;
      ...
      {showForm && (
        <Panel title={editing.id ? `Edit ${editing.id}` : "New sample"}>
          <div style={{ marginBottom: 14, padding: 12, borderRadius: 12, background: COLORS.bg, border: `1px solid ${COLORS.line}`, display: "flex", alignItems: "center", gap: 9 }}>
            ...
            <div style={{ fontSize: 12.5, color: COLORS.inkSoft }}><b style={{ color: COLORS.ink }}>Tip:</b> Upload a product/specification image from the Samples toolbar to let AI pre-fill these fields. You review everything before saving.</div>
          </div>
          <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
              ...
              <Field label="Cemboard color"><MaterialPicker label="Cemboard color" value={editing.cemboardColorId} onChange={(v) => setEditing({ ...editing, cemboardColorId: v })} options={cemboardColors} onAddNew={(name) => addMaterialListItem("cemboardColors", name)} onImport={() => setMaterialImportKey("cemboardColors")} /></Field>
      ...
      {materialImportKey && <MaterialImportModal
        tab={MATERIAL_LIST_TABS.find((t) => t.key === materialImportKey)}
        existingList={materialLists[materialImportKey] || []}
        onImport={(rows, options) => importMaterialListRows(materialImportKey, rows, options)}
        onClose={() => setMaterialImportKey(null)}
      />}
...
function normalizeMaterialHeader(value) {
  return String(value || "")
    .trim().toLowerCase()
    .replace(/[()\[\]{}]/g, "")
    .replace(/[._-]+/g, " ")
...
function materialImportField(headers, aliases) {
  const normalized = headers.map(normalizeMaterialHeader);
  for (const alias of aliases) {
    const i = normalized.indexOf(normalizeMaterialHeader(alias));
    if (i >= 0) return i;
  }
  return -1;
}

function MaterialImportModal({ tab, existingList, onImport, onClose }) {
  const [file, setFile] = useState(null);
  const [preview, setPreview] = useState([]);
  const [error, setError] = useState("");
  ...
  const readXlsx = async (f) => {
    const parsed = await parseXlsxWorkbook(f);
    if (!parsed.rows.length) throw new Error("The Excel file is empty.");
    const headerRow = parsed.rows[0];
    const headerCols = Object.keys(headerRow.cells || {}).sort((a, b) => a.localeCompare(b));
    const headers = headerCols.map((c) => headerRow.cells[c]);
  ...
  const chooseFile = async (e) => {
    ...
    e.target.value = "";
    if (!f) return;
    setFile(f); setPreview([]); setError(""); setReading(true);
    try {
      const lower = f.name.toLowerCase();
      const rows = lower.endsWith(".csv") ? await readCsv(f) : await readXlsx(f);
      if (!rows.length) throw new Error("No data rows were found.");
      const cleaned = rows.map((r) => ({ ...r, name: r.name || r.code || "" }));
      setPreview(cleaned);
    } catch (err) {
      setError(err?.message || String(err)); setFile(null); setPreview([]);
    } finally { setReading(false); }
  };

  const submit = () => {
    if (!preview.length) return;
    onImport(preview, { replaceExisting });
  };

  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(17,17,17,.30)", zIndex: 70, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "min(760px, 100%)", maxHeight: "90vh", background: COLORS.panel, borderRadius: 16, border: `1px solid ${COLORS.line}`, boxShadow: "0 20px 60px rgba(0,0,0,.18)", overflow: "hidden", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "15px 18px", borderBottom: `1px solid ${COLORS.line}`, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
          <div><div style={{ fontFamily: FONT_HEAD, fontSize: 18, fontWeight: 700 }}>Import {tab.label}</div><div style={{ marginTop: 3, fontSize: 12, color: COLORS.inkSoft }}>Upload Excel (.xlsx) or CSV. Recommended columns: <b>ID</b>, <b>Code</b>, <b>Name</b>.</div></div>
          <Button small variant="ghost" onClick={onClose}><X size={14} /></Button>
        </div>
        <div style={{ padding: 18, overflow: "auto", display: "flex", flexDirection: "column", gap: 14 }}>
          <label style={{ border: `1px dashed ${COLORS.woodDark}`, background: COLORS.amberSoft, borderRadius: 12, padding: 16, cursor: "pointer", display: "flex", alignItems: "center", gap: 12 }}>
            <FileSpreadsheet size={22} color={COLORS.woodDark} />
            <div style={{ flex: 1 }}><div style={{ fontWeight: 700 }}>{file ? file.name : "Choose material file"}</div><div style={{ fontSize: 12, color: COLORS.inkSoft, marginTop: 3 }}>{reading ? "Reading file…" : "Click to choose or replace the file"}</div></div>
            <input type="file" accept=".xlsx,.csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,text/csv" onChange={chooseFile} style={{ display: "none" }} />
          </label>

          {error && <div style={{ background: COLORS.redSoft, color: COLORS.red, border: `1px solid ${COLORS.red}55`, borderRadius: 10, padding: "10px 12px", fontSize: 13 }}>{error}</div>}

          <label style={{ display: "flex", alignItems: "flex-start", gap: 9, padding: "10px 12px", background: COLORS.bg, borderRadius: 10, cursor: "pointer" }}>
            <input type="checkbox" checked={replaceExisting} onChange={(e) => setReplaceExisting(e.target.checked)} style={{ marginTop: 3 }} />
            <div><div style={{ fontWeight: 700, fontSize: 13 }}>Replace current {tab.label.toLowerCase()}</div><div style={{ fontSize: 11.5, color: COLORS.inkSoft, marginTop: 2 }}>Use this only when the uploaded file is the new master list. Otherwise, imported rows will be merged with the current list.</div></div>
          </label>

          {preview.length > 0 && <div style={{ border: `1px solid ${COLORS.line}`, borderRadius: 12, overflow: "hidden" }}>
            <div style={{ padding: "10px 12px", background: COLORS.bg, fontSize: 12.5, fontWeight: 700 }}>{preview.length} row{preview.length === 1 ? "" : "s"} ready to import</div>
            <div style={{ maxHeight: 300, overflow: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}><thead><tr><th style={{ textAlign: "left", padding: "8px 10px", borderBottom: `1px solid ${COLORS.line}` }}>ID</th><th style={{ textAlign: "left", padding: "8px 10px", borderBottom: `1px solid ${COLORS.line}` }}>Code</th><th style={{ textAlign: "left", padding: "8px 10px", borderBottom: `1px solid ${COLORS.line}` }}>Name</th></tr></thead><tbody>{preview.slice(0, 100).map((r, i) => <tr key={i}><td style={{ padding: "7px 10px", borderBottom: `1px solid ${COLORS.line}` }}>{r.id || "—"}</td><td style={{ padding: "7px 10px", borderBottom: `1px solid ${COLORS.line}` }}>{r.code || "—"}</td><td style={{ padding: "7px 10px", borderBottom: `1px solid ${COLORS.line}` }}>{r.name || "—"}</td></tr>)}</tbody></table>
              {preview.length > 100 && <div style={{ padding: 9, color: COLORS.inkSoft, fontSize: 11.5 }}>Showing first 100 rows only.</div>}
            </div>
          </div>}
        </div>
        <div style={{ padding: "12px 18px", borderTop: `1px solid ${COLORS.line}`, display: "flex", justifyContent: "flex-end", gap: 8 }}>
          <Button variant="ghost" onClick={onClose}>Cancel</Button>
          <Button onClick={submit} disabled={!preview.length || reading}><Upload size={14} /> Import {preview.length ? `${preview.length} rows` : "data"}</Button>
        </div>
      </div>
    </div>
  );
...
function MaterialsView({ materialLists, saveList }) {
  ...
  const handleImport = (rows, options = {}) => {
    const base = options.replaceExisting ? [] : [...list];
    const byId = new Map(base.map((m) => [String(m.id || "").trim(), { ...m }]));
    const byCode = new Map(base.filter((m) => m.code).map((m) => [String(m.code).trim().toLowerCase(), String(m.id || "").trim()]));
    let generated = [...base];
    for (const raw of rows) {
      const code = String(raw.code || "").trim();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
        <div style={{ position: "relative", flex: 1, minWidth: 240 }}><Search size={15} style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: COLORS.inkSoft }} /><Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={`Search ${tab.label.toLowerCase()} by ID, code or name…`} style={{ paddingLeft: 32 }} /></div>
        <div style={{ fontSize: 12, color: COLORS.inkSoft }}>{filtered.length} of {list.length} rows</div>
      </div>
      ...
      <Panel>
        <Table columns={[{ key: "id", label: "ID" }, { key: "code", label: "Code" }, { key: "name", label: "Name" }, { key: "actions", label: "", align: "right", render: (m) => <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}><Button small variant="subtle" onClick={() => startEdit(m)}><Pencil size={13} /> Edit</Button><Button small variant="danger" onClick={() => remove(m.id)}><Trash2 size={13} /> Delete</Button></div> }]} rows={filtered} empty="Nothing added yet — import a material file or use the button above to add one." />
      </Panel>
