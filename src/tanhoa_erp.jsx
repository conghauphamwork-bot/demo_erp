...
import { LayoutDashboard, Users, FileText, ShoppingCart, Boxes, CalendarDays, Palette, Truck, Plus, X, Image as ImageIcon, Search, ListTodo, Download, Upload, FileSpreadsheet, CheckCircle2, AlertCircle } from "lucide-react";
...
const FONT_HEAD = '-apple-system, BlinkMacSystemFont, "SF Pro Display", "SF Pro Text", "Helvetica Neue", Arial, sans-serif';
const FONT_BODY = '-apple-system, BlinkMacSystemFont, "SF Pro Text", "Helvetica Neue", Arial, sans-serif';
    ...
    <div className="erp-shell" style={{ fontFamily: FONT_BODY, background: COLORS.bg, color: COLORS.ink }}>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .erp-sidebar { width: 220px; min-height: 100vh; flex: 0 0 220px; background: ${COLORS.sidebar}; color: #fff; padding: 24px 14px; }
        .erp-content { flex: 1 1 auto; min-width: 0; min-height: 100vh; height: 100vh; overflow: auto; padding: clamp(18px, 2.2vw, 32px); }
        ...
        @media (max-width: 1500px) {
          .erp-sidebar { width: 198px; flex-basis: 198px; }
          .sample-kanban-board {
          ...
          {NAV.map((n) => {
            ...
            return (
              <button
                ...
                style={{
                  textAlign: "left",
                  background: view === n.key ? "rgba(93,132,105,0.34)" : "transparent",
...
function SamplesView({ samples, saveSamples, customers, customerName, productTypes, productTypeName, orders, materialLists, saveMaterialList, materialPreps, saveMaterialPreps, tasks, saveTasks }) {
  ...
  const startNew = () => {
    setEditing({ ...BLANK_SAMPLE, customerId: customers[0]?.id || "", requiredComponents: [] , noteHistory: [] });
    setShowForm(true);
    setViewing(null);
  };
  const startEdit = (s) => {
