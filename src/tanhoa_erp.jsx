...
import { LayoutDashboard, Users, FileText, ShoppingCart, Boxes, CalendarDays, Palette, Truck, Plus, X, Image as ImageIcon, Search, ListTodo, Download, Upload, FileSpreadsheet, CheckCircle2, AlertCircle, Trash2, CheckSquare, Square, Move, QrCode as QrCodeIcon } from "lucide-react";
...
function AppInner() {
  ...
  if (qrSampleId) {
    const qrSample = samples.find((s) => s.id === qrSampleId);
    if (qrSample) {
      return <SampleQuickView sample={qrSample} customerName={customerName(qrSample.customerId)} productTypeName={productTypeName(qrSample.productTypeId)} materialLists={materialLists} materialPreps={materialPreps.filter((p) => p.sampleId === qrSample.id)} />;
    }
  }
...
function samplePublicUrl(sampleId) {
  ...
  return `${base}?sample=${encodeURIComponent(sampleId)}`;
}
