...
function exportFullBackup(data) {
  const backup = {
    backupVersion: 1,
    app: "Tân Hòa Outdoor Furniture — Sales ERP",
  ...
  a.href = url;
  a.download = `tanhoa_erp_backup_${stamp}.json`;
...
function SampleDetail({ sample: s, customerName, productTypeName, materialLists, materialPreps, saveMaterialPreps, allMaterialPreps, relatedTasks, allTasks, saveTasks, onEdit, onClose, onDelete, onSaveSample }) {
  ...
  const uploadProof = async (component, e) => {
    ...
    try { const dataUrl = await imageFileToDataUrl(file, 1200, 0.78); saveComponent(component, { photo: dataUrl }); }
