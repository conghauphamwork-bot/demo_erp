...
import { LayoutDashboard, Users, FileText, ShoppingCart, Boxes, CalendarDays, Palette, Truck, Plus, X, Image as ImageIcon, Search, ListTodo, Download, Upload, FileSpreadsheet, CheckCircle2, AlertCircle, Trash2, CheckSquare, Square, Move } from "lucide-react";
...
function xlsxSheetXml(headers, rows, imageColumnIndex) {
  ...
  const addRow = (rowNumber, values) => {
    ...
    cells.push(`<row r="${rowNumber}" ht="${rowNumber === 1 ? 24 : (imageColumnIndex >= 0 ? 82 : 22)}" customHeight="1">${rowCells}</row>`);
  };
  addRow(1, headers);
  rows.forEach((row, i) => addRow(i + 2, row.map((v, idx) => idx === imageColumnIndex ? "" : v)));
  const cols = headers.map((h, i) => `<col min="${i + 1}" max="${i + 1}" width="${i === imageColumnIndex ? 18 : Math.min(34, Math.max(12, String(h).length + 3))}" customWidth="1"/>`).join("");
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><dimension ref="A1:${columnLetters(maxCol)}${maxRow}"/><sheetViews><sheetView workbookViewId="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><cols>${cols}</cols><sheetData>${cells.join("")}</sheetData>${headers.length ? `<autoFilter ref="A1:${columnLetters(maxCol)}${maxRow}"/>` : ""}${imageColumnIndex >= 0 ? `<drawing r:id="rId1"/>` : ""}</worksheet>`;
}
...
async function downloadExcelWithEmbeddedImages(filename, headers, rows, imageColumnIndex, imageUrls) {
  const files = [];
  const sheetXml = xlsxSheetXml(headers, rows, imageColumnIndex);
  files.push({ name: "[Content_Types].xml", data: `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Default Extension="jpeg" ContentType="image/jpeg"/><Default Extension="png" ContentType="image/png"/><Default Extension="gif" ContentType="image/gif"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/>${imageColumnIndex >= 0 ? `<Override PartName="/xl/drawings/drawing1.xml" ContentType="application/vnd.openxmlformats-officedocument.drawing+xml"/>` : ""}<Override PartName="/xl/theme/theme1.xml" ContentType="application/vnd.openxmlformats-officedocument.theme+xml"/></Types>` });
  ...
  if (imageColumnIndex >= 0) {
    for (let i = 0; i < imageUrls.length; i++) {
      ...
      try {
        ...
        drawingImages.push({ row: i + 1, col: imageColumnIndex, mediaName });
        embeddedCount++;
      } catch (error) {
        failedImages++;
...
async function buildImportedSamples(file, currentSamples, customers, productTypes, materialLists) {
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    try {
      ...
      const image = parsed.imageByRow[row.rowNumber];
  ...
  const doImport = async () => {
    ...
    try {
      ...
      for (const item of parsed.results) {
        ...
        if (image) {
          try { clean.image = await importImageToStorage(image, clean.id); uploaded++; }
          catch (e) { console.warn("Imported image upload failed", e); }
        }
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ width: "min(980px, 100%)", maxHeight: "88vh", background: COLORS.panel, borderRadius: 18, border: `1px solid ${COLORS.line}`, boxShadow: "0 28px 90px rgba(0,0,0,.25)", overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: 18, overflowY: "auto", display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ padding: 14, borderRadius: 12, background: "#F5F1E9", border: `1px solid ${COLORS.line}` }}>
            ...
            <div style={{ fontSize: 12, color: COLORS.inkSoft, lineHeight: 1.6 }}>1) Download the template. 2) Keep the header row unchanged. 3) Fill one sample per row. 4) For Customer / Product Type / Materials, use the existing ID, code, or exact name from the ERP. 5) Put the sample photo into the Excel file as a real <b>Insert → Picture</b> object on the same row in <b>Image (embedded)</b>. 6) Upload the completed .xlsx here.</div>
          </div>
...
function SampleExportModal({ samples, customers, productTypes, materialLists, materialPreps, initialSampleId, onClose }) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const imageUrls = visibleSamples.map((sample) => sample.image || "");
      const result = await downloadExcelWithEmbeddedImages(`tanhoa-samples-${stamp}.xlsx`, headers, rows, imageColumnIndex, imageUrls);
      const message = result.failedImages
        ? `Excel exported. ${result.embeddedCount} image(s) embedded; ${result.failedImages} image(s) could not be embedded.`
        : `Excel exported successfully with ${result.embeddedCount} embedded image(s).`;
      alert(message);
      onClose();
    } catch (error) {
      console.error(error);
        ...
        <div style={{ padding: "14px 22px", overflowY: "auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "minmax(360px, 1.05fr) minmax(420px, 1.35fr)", gap: 18 }}>
            <Panel title="Columns & filters" action={<div style={{ display: "flex", gap: 6 }}><Button small variant="subtle" onClick={selectAll}>Select all</Button><Button small variant="ghost" onClick={clearAll}>Clear</Button></div>}>
              ...
              <div style={{ marginBottom: 10, padding: "8px 10px", borderRadius: 9, background: "#F5F1E9", color: COLORS.inkSoft, fontSize: 11.5 }}>Tip: select <b>Image (embedded)</b> to place the sample photo directly into the Excel file instead of exporting only a URL.</div>
...
function KanbanBoard({ samples, customerName, moveStage, onBulkMove, onBulkDelete, onAddSample, onCardClick }) {
          ...
          return (
            <div
              ...
              style={{
                ...
                borderRadius: 16, display: "flex", flexDirection: "column", minHeight: 0, overflow: "hidden",
            ...
            >
              ...
              <button
                ...
                style={{ margin: "0 10px 10px", padding: "9px 10px", borderRadius: 11, border: `1px solid ${theme.border}`, background: "rgba(255,255,255,.7)", color: theme.head, fontWeight: 750, fontSize: 11.5, textAlign: "center", cursor: "pointer", fontFamily: FONT_BODY, width: "calc(100% - 20px)" }}
              >
...
function KanbanCard({ sample: s, customerName, onClick, onToggleSelect, selected, onDragStart, onDragEnd, isDragging, onMoveStage, stageTheme }) {
  const ot = sampleOnTime(s);
...
function SampleDetail({ sample: s, customerName, productTypeName, materialLists, materialPreps, saveMaterialPreps, allMaterialPreps, relatedTasks, allTasks, saveTasks, onEdit, onClose, onDelete, onSaveSample, onExport }) {
  const { mainMaterials = [], finishes = [], woodSurface = [], fabricTypes = [], fabricColors = [], ropeTypes = [], ropeColors = [], cemboardColors = [] } = materialLists;
  ...
  const components = (s.requiredComponents && s.requiredComponents.length)
    ? s.requiredComponents
    : materialPreps.map((p) => ({ id: p.id, name: p.materialName || p.displayName || "Material", qty: p.qty || 1, targetDate: p.dueDate || "", status: p.status || "Waiting", photo: p.photo || "" }));
  ...
  return (
    <Panel title={`${s.id} — ${s.name}`} action={<div style={{ display: "flex", gap: 8, alignItems: "center" }}>{ot !== null && (ot ? <Badge tone="green">On time</Badge> : <Badge tone="red">Late</Badge>)}<Button small variant="subtle" onClick={onEdit}>Edit</Button><Button small variant="subtle" onClick={() => onExport(s.id)}><Download size={13} /> Export</Button><Button small variant="danger" onClick={onDelete}>Delete</Button><Button small variant="ghost" onClick={onClose}><X size={14} /> Close</Button></div>}>
      ...
      <div style={{ marginTop: 20 }}>
        ...
        {components.length === 0 ? <div style={{ color: COLORS.inkSoft, fontSize: 13.5, background: COLORS.bg, padding: 12, borderRadius: 8 }}>No required components selected. Edit the Sample and tick the components needed.</div> : (
          <div style={{ overflowX: "auto" }}><div style={{ minWidth: 760, display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1.4fr 80px 150px 150px 1.2fr", gap: 8, fontSize: 11.5, color: COLORS.inkSoft, padding: "0 10px" }}><div>Component</div><div>Qty</div><div>Target date</div><div>Status</div><div>Photo proof</div></div>
            {components.map((c) => (
              <div key={c.id || c.name} style={{ display: "grid", gridTemplateColumns: "1.4fr 80px 150px 150px 1.2fr", gap: 8, alignItems: "center", background: COLORS.bg, padding: "8px 10px", borderRadius: 8 }}>
...
function CalendarView({ samples, materialPreps, tasks, customerName }) {
  ...
  tasks.forEach((t) => {
    ...
    events.push({
      ...
      subtitle: t.sampleId ? (samples.find((s) => s.id === t.sampleId)?.erpNo || t.sampleId) : (t.referencePerson || "Follow-up"),
      sampleName: t.sampleId ? (samples.find((s) => s.id === t.sampleId)?.name || "") : "",
      sampleId: t.sampleId,
      taskId: t.id,
  ...
  const dueToday = events.filter((e) => e.date === today && !e.overdue);
  const thisWeek = events.filter((e) => e.date >= today && e.date <= weekEnd && !e.overdue);
  const completedKnown = samples.filter((s) => sampleOnTime(s) !== null);
  const onTimeCount = completedKnown.filter((s) => sampleOnTime(s)).length;
  const onTimeRate = completedKnown.length ? Math.round((onTimeCount / completedKnown.length) * 100) : null;
  ...
  const formatWeek = () => {
    ...
    if (am === bm) return `${am} ${a.getDate()}–${b.getDate()}, ${b.getFullYear()}`;
    return `${am} ${a.getDate()} – ${bm} ${b.getDate()}, ${b.getFullYear()}`;
  };

  const typeStyle = (e) => {
    if (e.overdue) return { bg: "#FFF0EE", border: "#FFB7AE", text: "#B53B2D", label: "Overdue" };
    if (e.status === "Done" || e.status === "Completed") return { bg: "#EAF6EE", border: "#B8DFC4", text: "#32724A", label: "Done" };
    if (e.type === "material") return { bg: "#FFF6E9", border: "#F4D3A7", text: "#9A5A2F", label: "Material" };
    if (e.type === "sample") return { bg: "#FFF0E8", border: "#FFC6A6", text: "#D94700", label: "Sample" };
    return { bg: "#FFF4EF", border: "#FFB89A", text: "#D94700", label: e.taskType || "Task" };
  };
    ...
    return (
      <button
        ...
        style={{
          ...
          borderRadius: 9, padding: compact ? "7px 8px" : "9px 10px", cursor: "pointer",
          boxShadow: "0 2px 7px rgba(0,0,0,.035)", transition: "transform .15s ease, box-shadow .15s ease",
        ...
        <div style={{ display: "flex", justifyContent: "space-between", gap: 6, alignItems: "flex-start" }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 3 }}>
              <span style={{ width: 6, height: 6, borderRadius: "50%", background: e.accent, flex: "0 0 auto" }} />
              <span style={{ fontSize: 9.5, fontWeight: 800, color: ts.text, textTransform: "uppercase", letterSpacing: ".04em" }}>{ts.label}</span>
            </div>
            <div style={{ fontSize: compact ? 11.5 : 12.5, lineHeight: 1.25, fontWeight: 750, color: COLORS.ink, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: compact ? 2 : 3, WebkitBoxOrient: "vertical" }}>{e.label}</div>
            {e.subtitle && <div style={{ marginTop: 3, fontSize: 10.5, color: COLORS.inkSoft, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{e.subtitle}</div>}
          </div>
          {!compact && <span style={{ fontSize: 10, color: COLORS.inkSoft }}>↗</span>}
        </div>
      </button>
    );
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14, minWidth: 0 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
        <div>
          ...
          <div style={{ color: COLORS.inkSoft, fontSize: 12.5, marginTop: 4 }}>See production targets and follow-up work in one weekly control board.</div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 10 }}>
        {[
          ["Overdue", overdue.length, COLORS.red, COLORS.redSoft],
          ["Due today", dueToday.length, COLORS.wood, COLORS.amberSoft],
          ["This week", thisWeek.length, COLORS.amber, COLORS.amberSoft],
          ["Sample on-time", onTimeRate === null ? "—" : `${onTimeRate}%`, COLORS.green, COLORS.greenSoft],
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
          <Button small variant="subtle" onClick={() => moveWeek(-7)}>‹</Button>
          <Button small variant="subtle" onClick={goToday}>Today</Button>
          <Button small variant="subtle" onClick={() => moveWeek(7)}>›</Button>
          <div style={{ fontSize: 16, fontWeight: 800, marginLeft: 4 }}>{formatWeek()}</div>
        </div>
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {[['all','All'],['task','Follow-up'],['sample','Samples'],['material','Materials']].map(([k,l]) => <button key={k} onClick={() => setFilter(k)} style={{ border: `1px solid ${filter === k ? COLORS.wood : COLORS.line}`, background: filter === k ? COLORS.amberSoft : COLORS.panel, color: filter === k ? COLORS.woodDark : COLORS.inkSoft, borderRadius: 999, padding: "6px 10px", fontSize: 11.5, fontWeight: 700, cursor: "pointer" }}>{l}</button>)}
        </div>
      </div>
      ...
      {view === "week" ? (
        <div style={{ background: COLORS.panel, border: `1px solid ${COLORS.line}`, borderRadius: 14, overflow: "hidden", boxShadow: "0 2px 10px rgba(0,0,0,.035)" }}>
          <div style={{ display: "grid", gridTemplateColumns: "220px repeat(7, minmax(0, 1fr))", borderBottom: `1px solid ${COLORS.line}` }}>
            <div style={{ padding: "11px 14px", background: "#FAFAFA", borderRight: `1px solid ${COLORS.line}` }}>
              <div style={{ fontSize: 10, fontWeight: 800, color: COLORS.inkSoft, textTransform: "uppercase", letterSpacing: ".08em" }}>Production / Follow-up</div>
              <div style={{ fontSize: 10.5, color: COLORS.inkSoft, marginTop: 3 }}>Target & deadline control</div>
            </div>
            {weekDays.map((d) => {
              const isToday = d.iso === today;
              const count = (eventsByDate[d.iso] || []).length;
              return <div key={d.iso} style={{ padding: "8px 8px 9px", textAlign: "center", background: isToday ? COLORS.wood : "#FAFAFA", color: isToday ? "#fff" : COLORS.ink }}>
                <div style={{ fontSize: 10, fontWeight: 700, opacity: .78 }}>{d.date.toLocaleDateString("en-US", { weekday: "short" })}</div>
                <div style={{ fontSize: 17, fontWeight: 850, lineHeight: 1.15 }}>{d.date.getDate()}</div>
                <div style={{ fontSize: 9.5, opacity: .72 }}>{d.date.toLocaleDateString("en-US", { month: "short" })} · {count}</div>
              </div>;
            })}
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "220px repeat(7, minmax(0, 1fr))", minHeight: 430 }}>
            <div style={{ background: "#FAFAFA", borderRight: `1px solid ${COLORS.line}` }}>
              <div style={{ padding: 12, borderBottom: `1px solid ${COLORS.line}`, minHeight: 150 }}>
                <div style={{ fontSize: 12.5, fontWeight: 800 }}>Follow-up queue</div>
                <div style={{ fontSize: 10.5, color: COLORS.inkSoft, marginTop: 3 }}>Items requiring attention</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 7, marginTop: 13 }}>
                  {overdue.slice(0, 3).map((e) => <div key={e.id} style={{ fontSize: 10.5, display: "flex", gap: 6, alignItems: "center" }}><span style={{ width: 6, height: 6, borderRadius: "50%", background: COLORS.red }} /><span style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{e.label}</span></div>)}
                  {!overdue.length && <div style={{ fontSize: 10.5, color: COLORS.green }}>No overdue items.</div>}
                </div>
              </div>
              <div style={{ padding: 12 }}>
                <div style={{ fontSize: 10.5, fontWeight: 800, color: COLORS.inkSoft, textTransform: "uppercase", letterSpacing: ".06em" }}>Legend</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 7, marginTop: 9, fontSize: 10.5, color: COLORS.inkSoft }}>
                  <div><span style={{ color: COLORS.wood, fontWeight: 800 }}>●</span> Follow-up</div>
                  <div><span style={{ color: COLORS.amber, fontWeight: 800 }}>●</span> Material</div>
                  <div><span style={{ color: COLORS.green, fontWeight: 800 }}>●</span> Completed</div>
                  <div><span style={{ color: COLORS.red, fontWeight: 800 }}>●</span> Overdue</div>
                </div>
              </div>
            </div>
            {weekDays.map((d) => {
              const dayEvents = eventsByDate[d.iso] || [];
              return <div key={d.iso} style={{ padding: 7, borderRight: `1px solid ${COLORS.line}`, background: d.iso === today ? "#FFF9F5" : "#FFFFFF", minWidth: 0 }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                  {dayEvents.slice(0, 5).map((e) => <EventCard key={e.id} e={e} compact />)}
                  {dayEvents.length > 5 && <button onClick={() => setSelectedEvent({ date: d.iso, label: `${dayEvents.length - 5} more items`, type: "more", more: dayEvents.slice(5) })} style={{ border: "none", background: "transparent", color: COLORS.wood, fontWeight: 800, fontSize: 10.5, cursor: "pointer", padding: 4 }}>+ {dayEvents.length - 5} more</button>}
                  {!dayEvents.length && <div style={{ minHeight: 120, display: "grid", placeItems: "center", color: "#B8BCC2", fontSize: 10.5 }}>No plan</div>}
                </div>
              </div>;
            })}
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 14 }}>
          <Panel title="Agenda — this week">
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {visibleEvents.filter((e) => e.date >= weekDays[0].iso && e.date <= weekEnd).map((e) => <div key={e.id} style={{ display: "grid", gridTemplateColumns: "82px 1fr", gap: 10, alignItems: "start" }}><div style={{ fontSize: 10.5, color: COLORS.inkSoft, fontWeight: 700 }}>{e.date}</div><EventCard e={e} /></div>)}
              {!visibleEvents.some((e) => e.date >= weekDays[0].iso && e.date <= weekEnd) && <div style={{ color: COLORS.inkSoft, fontSize: 13 }}>Nothing scheduled this week.</div>}
            </div>
          </Panel>
          <Panel title="Follow-up queue">
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {overdue.slice(0, 8).map((e) => <EventCard key={e.id} e={e} />)}
              {!overdue.length && <div style={{ color: COLORS.green, fontSize: 13 }}>Nothing overdue — all caught up.</div>}
            </div>
          </Panel>
        </div>
      )}
      ...
      {selectedEvent && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(17,17,17,.28)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={() => setSelectedEvent(null)}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "min(520px, 100%)", background: COLORS.panel, borderRadius: 16, border: `1px solid ${COLORS.line}`, boxShadow: "0 20px 60px rgba(0,0,0,.18)", overflow: "hidden" }}>
            <div style={{ padding: "15px 17px", borderBottom: `1px solid ${COLORS.line}`, display: "flex", justifyContent: "space-between", gap: 12 }}>
              <div><div style={{ fontSize: 10, color: COLORS.wood, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".08em" }}>{selectedEvent.type === "more" ? "Scheduled items" : typeStyle(selectedEvent).label}</div><div style={{ fontSize: 18, fontWeight: 850, marginTop: 3 }}>{selectedEvent.label}</div></div>
              <button onClick={() => setSelectedEvent(null)} style={{ border: "none", background: COLORS.bg, borderRadius: 8, width: 30, height: 30, cursor: "pointer" }}><X size={15} /></button>
            </div>
            <div style={{ padding: 17 }}>
              {selectedEvent.type === "more" ? <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{selectedEvent.more.map((e) => <EventCard key={e.id} e={e} />)}</div> : <>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  <div style={{ background: COLORS.bg, borderRadius: 10, padding: 10 }}><div style={{ fontSize: 10, color: COLORS.inkSoft }}>Date</div><div style={{ marginTop: 3, fontWeight: 750 }}>{selectedEvent.date}</div></div>
                  <div style={{ background: COLORS.bg, borderRadius: 10, padding: 10 }}><div style={{ fontSize: 10, color: COLORS.inkSoft }}>Status</div><div style={{ marginTop: 3, fontWeight: 750 }}>{selectedEvent.status || "—"}</div></div>
                </div>
                <div style={{ marginTop: 10, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  <div><div style={{ fontSize: 10, color: COLORS.inkSoft }}>ERP / Reference</div><div style={{ marginTop: 3, fontSize: 12.5, fontWeight: 700, wordBreak: "break-word" }}>{selectedEvent.subtitle || "—"}</div></div>
                  <div><div style={{ fontSize: 10, color: COLORS.inkSoft }}>Sample</div><div style={{ marginTop: 3, fontSize: 12.5, fontWeight: 700 }}>{selectedEvent.sampleName || selectedEvent.sampleId || "—"}</div></div>
                </div>
...
function TaskKanbanBoard({ tasks, sampleLabel, moveStatus, onCardClick }) {
  ...
  return (
    <div className="kanban-board" style={{ display: "flex", gap: 14, overflowX: "auto", paddingBottom: 8 }}>
