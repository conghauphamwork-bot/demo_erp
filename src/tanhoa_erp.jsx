...
function CalendarView({ samples, materialPreps, tasks, customerName, saveSamples, saveMaterialPreps, saveTasks }) {
  ...
  const saveEditing = () => {
    ...
    if (e.type === "sample") {
      ...
      saveMaterialPreps(materialPreps.map((p) => p.id === e.prepId ? { ...p, dueDate: editing.date, status: editing.status } : p));
    } else if (e.type === "task") {
  ...
  const markDone = () => {
    ...
    if (e.type === "material") saveMaterialPreps(materialPreps.map((p) => p.id === e.prepId ? { ...p, status: "Done" } : p));
