    ...
    <div className="erp-shell" style={{ fontFamily: FONT_BODY, background: COLORS.bg, color: COLORS.ink }}>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .kanban-board { display: grid !important; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 14px; overflow: visible !important; }
        .kanban-column { min-width: 0 !important; width: auto !important; flex: 1 1 0 !important; max-height: calc(100vh - 220px) !important; }
        .kanban-column > div:last-child { min-height: 0; }
        @media (max-width: 1250px) {
          .erp-sidebar { width: 198px; flex-basis: 198px; }
          .kanban-board { grid-template-columns: repeat(2, minmax(280px, 1fr)); overflow-x: auto !important; }
        ...
        @media (max-width: 820px) {
          ...
          .kanban-board { grid-template-columns: 1fr; overflow: visible !important; }
          .kanban-column { min-width: 0 !important; max-height: none !important; }
        }
...
function SamplesView({ samples, saveSamples, customers, customerName, productTypes, productTypeName, orders, materialLists, saveMaterialList, materialPreps, saveMaterialPreps, tasks, saveTasks }) {
  ...
  const startEdit = (s) => {
    ...
    setEditing({ ...s, requiredComponents, noteHistory: s.noteHistory || (s.notes ? [{ id: "legacy-note", text: s.notes, createdAt: new Date().toISOString() }] : []) });
...
function SampleDetail({ sample: s, customerName, productTypeName, materialLists, materialPreps, saveMaterialPreps, allMaterialPreps, relatedTasks, allTasks, saveTasks, onEdit, onClose, onDelete, onSaveSample }) {
  ...
  return (
    <Panel title={`${s.id} — ${s.name}`} action={<div style={{ display: "flex", gap: 8, alignItems: "center" }}>{ot !== null && (ot ? <Badge tone="green">On time</Badge> : <Badge tone="red">Late</Badge>)}<Button small variant="subtle" onClick={onEdit}>Edit</Button><Button small variant="danger" onClick={onDelete}>Delete</Button><Button small variant="ghost" onClick={onClose}><X size={14} /> Close</Button></div>}>
