...
import { LayoutDashboard, Users, FileText, ShoppingCart, Boxes, CalendarDays, Palette, Truck, Plus, X, Image as ImageIcon, Search, ListTodo, Download, Upload, FileSpreadsheet, CheckCircle2, AlertCircle, Trash2, CheckSquare, Square, Move, Clock3, CircleDot, CircleCheckBig, MoreVertical } from "lucide-react";
...
const TASK_STATUSES = ["To Do", "In Progress", "Pending", "Done"];
const TASK_STATUS_NORMALIZE = { Waiting: "Pending", "To Do": "To Do", "In Progress": "In Progress", Pending: "Pending", Done: "Done" };
...
function taskStatusTone(s) {
  return { "To Do": "neutral", "In Progress": "wood", Pending: "amber", Waiting: "amber", Done: "green" }[s] || "neutral";
}
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setTasks(data.tasks.map((t) => ({ ...BLANK_TASK, ...t, status: TASK_STATUS_NORMALIZE[t.status] || "To Do" })));
...
function CalendarView({ samples, materialPreps, tasks, customerName }) {
  ...
  materialPreps.forEach((p) => {
    if (p.dueDate) {
      ...
      events.push({
        ...
        date: p.dueDate,
        type: "material",
        label: `${p.materialName || "Material"} — ${sample?.erpNo || p.sampleId}${sample?.name ? ` · ${sample.name}` : ""}`,
        sampleId: p.sampleId,
        prepId: p.id || "",
        componentField: p.componentField || "",
        status: p.status,
  ...
  const eventTypeLabel = (ev) => ({ material: "Material target", sample: "Sample target", task: ev.taskType || "Task" }[ev.type] || ev.type);

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <h1 style={{ fontFamily: FONT_HEAD, fontSize: 26, margin: 0 }}>Master Production Calendar</h1>
          <div style={{ color: COLORS.inkSoft, fontSize: 12.5, marginTop: 4 }}>Material target dates, sample target dates, and task deadlines are synced from their source records.</div>
        </div>
      </div>
...
function TaskKanbanBoard({ tasks, sampleLabel, moveStatus, onCardClick }) {
  ...
  const byStatus = (status) => tasks.filter((t) => (TASK_STATUS_NORMALIZE[t.status] || t.status) === status);
  const statusMeta = {
    "To Do": { icon: CircleDot, subtitle: "Tasks to be started", tone: "#A7AFB8", soft: "#F4F5F6", line: "#E2E5E8" },
    "In Progress": { icon: Clock3, subtitle: "Currently working on", tone: COLORS.wood, soft: "#FFF2EA", line: "#FFD5C2" },
    "Pending": { icon: MoreVertical, subtitle: "Waiting for follow-up", tone: "#D98443", soft: "#FFF6EF", line: "#F3D7C3" },
    "Done": { icon: CircleCheckBig, subtitle: "Completed tasks", tone: COLORS.green, soft: "#EEF8F1", line: "#CDE8D5" },
  };
        ...
        return (
          <div
            ...
            style={{
              ...
              minHeight: 520,
              maxHeight: 820,
              background: isOver ? meta.soft : "#F1F3F5",
              border: `1px solid ${isOver ? meta.tone : "#E2E5E8"}`,
              borderRadius: 14,
              display: "flex",
              flexDirection: "column",
              overflow: "hidden",
              boxShadow: isOver ? `0 0 0 3px ${meta.tone}18` : "0 2px 8px rgba(17,17,17,.025)",
              transition: "background .16s ease, border-color .16s ease, box-shadow .16s ease",
            }}
          >
            <div style={{ padding: "14px 15px 12px", background: "rgba(255,255,255,.72)", borderBottom: `1px solid ${meta.line}` }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 9, minWidth: 0 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 8, display: "grid", placeItems: "center", background: meta.soft, color: meta.tone, border: `1px solid ${meta.line}`, flex: "0 0 auto" }}>
                    <Icon size={16} strokeWidth={1.9} />
                  </div>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 14.5, fontWeight: 750, color: COLORS.ink, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{status}</div>
                    <div style={{ fontSize: 10.5, color: COLORS.inkSoft, marginTop: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{meta.subtitle}</div>
                  </div>
                </div>
                <span style={{ minWidth: 26, height: 26, padding: "0 7px", display: "inline-flex", alignItems: "center", justifyContent: "center", borderRadius: 999, background: "#fff", border: `1px solid ${meta.line}`, color: COLORS.inkSoft, fontSize: 11.5, fontWeight: 750 }}>{cards.length}</span>
              </div>
              <div style={{ height: 3, borderRadius: 999, background: "#E7E9EB", marginTop: 12, overflow: "hidden" }}>
                <div style={{ height: "100%", width: cards.length ? "100%" : "0%", maxWidth: "100%", borderRadius: 999, background: meta.tone, opacity: .72 }} />
              </div>
            </div>

            <div className="task-kanban-column-body" style={{ padding: 10, display: "flex", flexDirection: "column", gap: 9, overflowY: "auto", flex: 1, minHeight: 0 }}>
              {cards.map((t) => (
                <div
                  key={t.id}
                  draggable
                  ...
                  style={{
                    ...
                    padding: 11,
                    cursor: "grab",
                    boxShadow: "0 2px 7px rgba(17,17,17,.045)",
                    opacity: draggingId === t.id ? .55 : 1,
                  }}
                >
                  <div onClick={() => onCardClick(t)} style={{ cursor: "pointer" }}>
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                      <div style={{ display: "flex", gap: 8, minWidth: 0 }}>
                        <div style={{ width: 10, height: 10, borderRadius: "50%", background: meta.tone, marginTop: 4, flex: "0 0 auto" }} />
                        <div style={{ minWidth: 0 }}>
                          <div style={{ fontSize: 13, fontWeight: 700, lineHeight: 1.35, color: COLORS.ink }}>{t.name}</div>
                          {t.referencePerson && <div style={{ fontSize: 11.2, color: COLORS.inkSoft, marginTop: 4, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{t.referencePerson}</div>}
                        </div>
                      </div>
                      <span style={{ color: "#9AA0A6", fontSize: 16, lineHeight: 1 }}>⋯</span>
                    </div>

                    {sampleLabel(t.sampleId) && (
                      <div style={{ marginTop: 8, paddingTop: 7, borderTop: `1px solid #F0F1F2`, fontSize: 10.8, color: COLORS.teal, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        🔗 {sampleLabel(t.sampleId)}
                      </div>
                    )}
                  ...
                  <select
                    value={TASK_STATUS_NORMALIZE[t.status] || t.status}
                    onChange={(e) => moveStatus(t.id, e.target.value)}
                    onClick={(e) => e.stopPropagation()}
                    style={{ marginTop: 9, width: "100%", fontSize: 11.5, fontWeight: 600, padding: "7px 9px", borderRadius: 8, border: `1px solid ${meta.line}`, background: meta.soft, color: meta.tone, fontFamily: FONT_BODY, cursor: "pointer" }}
                  >
              ...
              {cards.length === 0 && (
                <div style={{ flex: 1, minHeight: 110, display: "grid", placeItems: "center", textAlign: "center", color: COLORS.inkSoft, fontSize: 12 }}>
                  <div><div style={{ fontWeight: 650, color: "#7B8086" }}>No tasks</div><div style={{ marginTop: 3, fontSize: 10.5 }}>Drop a task here</div></div>
                </div>
              )}
            </div>

            <div style={{ padding: "10px", borderTop: `1px solid ${meta.line}`, background: "rgba(255,255,255,.52)" }}>
              <button
                type="button"
                onClick={() => onCardClick({ ...BLANK_TASK, status })}
                style={{ width: "100%", border: `1px dashed ${meta.line}`, background: "rgba(255,255,255,.65)", color: meta.tone, borderRadius: 9, padding: "8px 10px", fontFamily: FONT_BODY, fontSize: 12, fontWeight: 700, cursor: "pointer" }}
              >
                <Plus size={13} style={{ verticalAlign: "-2px", marginRight: 5 }} /> Add task
              </button>
            </div>
          </div>
        );
